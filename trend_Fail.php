...
*/
      ...
      $(function initChartTrend() {
        var checkDate =  $.trim($("#check_fail").text());
...
/*
                      ...
                      //  resizable: false,
                       ...
                       // modal: true,
                        ...
                        hide: {
                         ...
                         title: 'Chart of  fail  Trend(' + dialog_title_yield_trend + ')',
                         position: { my: "center", at: "top", of:  window}
                      });
            
                       $( "#getTrendFail" ).dblclick(function() {
                         $( "#diago_failTrend" ).dialog( "open" );
                         var bar_url_string;
                        var condition = $("#conditionBar").text();
                        condition = condition.replace(/\anda/g,"\#").replace(/\and/g,"&").replace(/ /g,"").replace(/'/g,"").replace(/\#/g,"anda").replace(/\Date/g,"time");
                        condition =  condition.toLowerCase();
                        if(condition=="")
                       bar_url_string = "barFailTrend?time="+time_var+"&item=" + item_var +"&section=FATP";
                      else
                      bar_url_string = "barFailTrend.php?item=" + item_var +"&"+condition;
                       $("#diago_failTrend_Bar").load(bar_url_string);
                      });
        ...
        $.get(url_string, function(data) {
             ...
             //   html_str += getLegendString(item_var, project_var, section_var, factory_var, line_var, station_var);
               ...
               if (+today_var > +yesterday_var)
                ...
                $("#chart_trend_fail").html(html_str);
 ...
 var chartFail= new Highcharts.Chart({
            //    $("#chart_trend_div_input").highcharts({
                  chart: {
                   renderTo:'diago_failTrend_line',
                    type: 'line'
               //     width: 500,
                //    height: height_var
                  },
...
//                        text: unit_str
    ...
    <div id="noData_fail" style="display:none">
        <img src="pic/noData/retestRatio.bmp" title="No Data on <?php echo $_GET["time"]; ?>" />
    </div>
    <div id="chart_trend_fail" style="display:none"></div>
